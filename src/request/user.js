 ...
 * '/user/userlogin' -- 接口url，若需要在url中拼接其他信息：
 *                  首先需要在传参处改为(p, other1, other2)
 *                  然后将url改为`/user/${other1}/userlogin/${other2}`
...
// 用户登录
export const login = (p) => post('/user/userlogin', p)
// 用户注册
export const addUser = (p) => post('/user/adduser', p)
// 用户退出登录
export const logout = (p) => post('/user/userlogout', p)
// 获取登录状态及用户信息
